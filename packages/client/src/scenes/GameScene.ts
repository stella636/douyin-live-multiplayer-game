...
import { GAME_CONFIG, MVP_RULES, type PlayerSnapshot } from "@douyin-game/shared";
...
export class GameScene extends Phaser.Scene {
  private room?: Room<RoomStateLike>;
  private readonly avatars = new Map<string, PlayerAvatar>();
  private readonly playerName = `玩家${Math.floor(Math.random() * 900 + 100)}`;
  private hud?: Phaser.GameObjects.Text;
  private help?: Phaser.GameObjects.Text;
  private roundResult?: Phaser.GameObjects.Text;
  ...
  private cameraHitBox = false;
  private cameraHeadTopX: number | null = null;
  private cameraHeadTopY: number | null = null;
  private cameraFootCenterX: number | null = null;
  private cameraGroundY: number | null = null;
  private lastCameraHitAt = 0;
  private readonly groundLootEntries: GroundLootEntry[] = [];
  private groundGuide?: Phaser.GameObjects.Ellipse;
  ...
  async create() {
    ...
    this.room = await client.joinOrCreate<RoomStateLike>(GAME_CONFIG.roomName, {
    ...
    this.room.onMessage("box:consumed", (payload) => {
      ...
      const lootDrop = avatar?.applyConsumedCount(payload.boxCount) as LootDropResult | undefined;
  ...
  private renderFromState(state: RoomStateLike) {
    const seenIds = new Set<string>();
    const players = state.players as PlayerSnapshot[];
  ...
  private async triggerMockGift(mode: "self" | "random") {
    if (!this.room) {
      return;
    }
  ...
  private playLocalHitFx(boxCount: number, lootDrop?: LootDropResult) {
    this.playCoinChime();
    if (lootDrop) {
  ...
  private dropLootNearFeet(loot: LootDropResult) {
    const burstX = GAME_CONFIG.mapWidth / 2;
    const floorY = this.getEstimatedGroundY();
    const spreadX = loot.side === "left" ? -1 : 1;
    const footBiasX =
      this.cameraFootCenterX !== null ? (this.cameraFootCenterX - 0.5) * GAME_CONFIG.mapWidth * 0.18 : 0;
  ...
  private createGroundLootIcon(type: string) {
